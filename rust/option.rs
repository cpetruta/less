...
/// Global state
struct ScanContext {
    pending: Option<&'static mut LOption>,
    plus_option: bool,
    less_is_more: bool,
}
...
unsafe fn scan_option<'a>(ctx: &'a mut ScanContext, mut s: &'a str, is_env: bool) {
    ...
    while !s.is_empty() {
        ...
        match optc {
            ...
            '0'..='9' => {
                // This digit is the option character, treat it as "-0z" style
                // Just continue with the digit as optc
                optc = c.chars().next().unwrap();
                // Skip the rest and let it be processed normally
                continue;
            }
        ...
        /* Lookup option */
        let opt_ptr: *mut LOption;
        let print_name: &str;

        if let Some(name) = opt_name.take() {
            match findopt_name(name, None) {
                (Some(o), _) => { opt_ptr = o; print_name = name; },
                (None, _) => {
                    error(
            ...
            match findopt(optc) {
                Some(o) => { opt_ptr = o; print_name = optc.to_string().as_str(); },
                None => {
                    error(
                        b"No such option" as *const u8 as *const std::ffi::c_char,
                        0 as *mut std::ffi::c_void as *mut PARG,
        ...
        match opt.otype {
            ...
            OptFlags::TRIPLE => {
                if let Some(mut v) = opt.ovar {
                    if set_default {
                        ...
                        v = flip_triple(opt.odefault, optc.is_lowercase());
                    }
                }
            }
            ...
            OptFlags::STRING => {
                ...
                if let Some((val, rest)) = optstring(s, "", Some(opt.odesc[1])) {
                    arg = Some(val);
                    s = rest;
                }
            }
...
pub unsafe fn getfraction<'a>(
    ...
    if first.map_or(true, |c: char| !c.is_ascii_digit()) {
        //TODO properly handle num_error
        return (None, s);
    }
...
pub unsafe extern "C" fn init_unsupport() {
    ...
    loop {
        let mut opt_ptr: Option<*mut LOption> = None;
        s = s.trim_start();
        if s.is_empty() {
            break;
        }
        ...
        if s.starts_with("-") {
            s = &s[1..];
            (opt_ptr, _) = findopt_name(s, None);
        } else {
            opt_ptr = findopt(s.chars().next().unwrap());
            if !opt_ptr.is_none() {
                s = &s[1..];
            }
        }
        if !opt_ptr.is_none() {
            unsafe { &mut *opt_ptr.unwrap() }.otype |= OptFlags::UNSUPPORTED;
        }
    }
}
