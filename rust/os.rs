...
extern "C" {
    ...
    fn __sigsetjmp(__env: *mut __jmp_buf_tag, __savemask: std::ffi::c_int) -> std::ffi::c_int;
...
unsafe extern "C" fn check_poll(
    ...
    if getting_one_screen as std::ffi::c_uint != 0
        && get_time() < less_start_time + (screenfill_ms / 1000 as std::ffi::c_int) as time_t
    {
    ...
    if poller[1 as std::ffi::c_int as usize].revents as std::ffi::c_int & 0x1 as std::ffi::c_int
        != 0
    {
...
pub unsafe extern "C" fn iread(
    ...
    let mut n: ssize_t = 0;
    loop {
        if reading as u64 == 0 && __sigsetjmp(read_label.as_mut_ptr(), 1 as std::ffi::c_int) != 0 {
            reading = LFALSE;
...
pub unsafe extern "C" fn iopen(
    ...
    while opening as u64 == 0 && __sigsetjmp(open_label.as_mut_ptr(), 1 as std::ffi::c_int) != 0 {
        opening = LFALSE;
        if sigs
            & ((1 as std::ffi::c_int) << 0 as std::ffi::c_int
                | (1 as std::ffi::c_int) << 1 as std::ffi::c_int)
...
#[no_mangle]
pub unsafe extern "C" fn signal_message(mut sig: std::ffi::c_int) -> *const std::ffi::c_char {
...
#[no_mangle]
pub unsafe extern "C" fn umuldiv(mut val: uintmax, mut num: uintmax, mut den: uintmax) -> uintmax {
...
#[no_mangle]
pub unsafe extern "C" fn percentage(mut num: POSITION, mut den: POSITION) -> std::ffi::c_int {
    return umuldiv(
        num as uintmax,
        100 as std::ffi::c_int as uintmax,
        den as uintmax,
    ) as std::ffi::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn percent_pos(
    ...
    let mut pctden: POSITION =
        (percent * 1000000 as std::ffi::c_int) as std::ffi::c_long + fraction;
    return umuldiv(
        pos as uintmax,
        pctden as uintmax,
